...
<div class="px-5 lg:px-0 lg:mx-[auto] container flex items-center justify-between">
    ...
    <div class="flex items-center justify-end gap-5">
        ...
        <div class="py-1 flex justify-end">
              @guest
              <a href="{{url('signin')}}" class="cursor-pointer rounded-lg px-3 py-2.5 text-sm font-medium text-gray-50 hover:bg-gray-50 hover:text-gray-700">Log in &#8594;</a>
              @endguest
              @auth
              <div>
                        <form method="POST" action="{{url('/')}}" class="flex justify-end">
                        @csrf
                        <input type="submit" class="cursor-pointer rounded-lg px-2.5 py-2.5 text-sm font-medium text-gray-50 hover:bg-gray-50 hover:text-gray-700" value="Log out &#8594;">
                        </form>
              </div>
              @endauth
        </div>
